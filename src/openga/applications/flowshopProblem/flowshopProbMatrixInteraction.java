 ...
 * @version 1.0
 */
public class flowshopProbMatrixInteraction extends flowshopSGA {

    public flowshopProbMatrixInteraction() {
    }
    /***
     * AC2 parameters
     */
...
//        constructInitialSolutions(Population, readNEHInstancePop1);
        ...
        implementationResult += fileName + "\t" + startingGenerationPercent + "\t" + intervalPercent + "\t" + DEFAULT_PopSize + "\t" + DEFAULT_crossoverRate + "\t" + DEFAULT_mutationRate + "\t" + lamda + "\t" + beta + "\t" + GaMain.getArchieve().getSingleChromosome(bestInd).getObjValue()[0] + "\t" + timeClock1.getExecutionTime() / 1000.0 + "\n";
        writeFile("flowshop_ProbMatrix_interactive_0528_comp", implementationResult);
    ...
    public static void main(String[] args) {
        ...
        int popSize[] = new int[]{400};//50, 100, 155, 210, 500 {50: Taillard}
        int counter = 0;
        double crossoverRate[] = new double[]{0.9},//0.6, 0.9 {0.9}
                mutationRate[] = new double[]{0.4},//0.1, 0.5 {0.5}
                elitism = 0.2;
        ...
        int jobs[] = new int[]{200};//20, 50, 100, 200,    500
                                                ...
                                                for (int lx = 0; lx < lamdalearningrate.length; lx++) {
                                                    for (int bx = 0; bx < betalearningrate.length; bx++) {
                                                        if ((jobs[j] <= 100) || (jobs[j] == 200 && machines[s] >= 10) || (jobs[j] == 500 && machines[s] == 20)) {
                                                            flowshopProbMatrixInteraction flowshop1 = new flowshopProbMatrixInteraction();
