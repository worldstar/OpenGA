 ...
 * @version 1.0
 */
public class flowshopProbMatrixforPREDA extends flowshopSGA{

    public flowshopProbMatrixforPREDA() {
    }
    /***
     * AC2 parameters
     */
    ...
    public void initiateVars() {
        GaMain = new singleThreadGAwithProbabilityMatrixPREDA(); //singleThreadGAwithProbabilityMatrix singleThreadGAwithProbabilityMatrixInitialPop  singleThreadGAwithProbabilityMatrixIL singleThreadGAwithProbabilityMatrixInteraction
        ...
        DEFAULT_generations =  totalSolnsToExamine / (DEFAULT_PopSize);    //    totalSolnsToExamine / (DEFAULT_PopSize)
...
//        implementationResult += fileName + "\t" + NEH_makespan + "\t" + NEHLS_makespan  + "\t" + GaMain.getArchieve().getSingleChromosome(bestInd).getObjValue()[0] + "\t" + timeClock1.getExecutionTime() / 1000.0 + "\n";
        writeFile("flowshop_ProbMatrix_PREDA_1001_50", implementationResult);
...
//        NEHLS_makespan = calcObjectives(sequenceLS);
    ...
    public static void main(String[] args) {
        ...
        int popSize[] = new int[]{500};//50, 100, 155, 210, 500 {400}  10n
        int counter = 0;
                                                ...
                                                for (int lx = 0; lx < lamdalearningrate.length; lx++) {
                                                    for (int bx = 0; bx < betalearningrate.length; bx++) {
                                                        if ((jobs[j] <= 100) || (jobs[j] == 200 && machines[s] >= 10) || (jobs[j] == 500 && machines[s] == 20)) {
                                                            flowshopProbMatrixforPREDA flowshop1 = new flowshopProbMatrixforPREDA();
                                                            ...
                                                            totalSolnsToExamine = (jobs[j] * jobs[j] * 2 * 500);  //for compare 13 EDA of 2011review
