 ...
 * @version 1.1
 */
public class flowshop_OASPSD_ThreadPool implements Runnable {

  public flowshop_OASPSD_ThreadPool() {
...
//    
//    int cores = 6;//Math.max(1, Runtime.getRuntime().availableProcessors() - 2)
//    
...
//                flowshop_OASPSD_ThreadPool flowshop1 = new flowshop_OASPSD_ThreadPool();
...
//  }
  public static void main(String[] args) throws IOException, ParseException {
    ...
    crossoverRate = new double[]{0.5, 0.9};  //0.5,0.9
    mutationRate = new double[]{0.1, 0.5};   //0.1,0.5
    int counter = 0;
    double elitism[] = new double[]{0.2};
    int generations[] = new int[]{1000};
              ...
              for (int o = 0; o < elitism.length; o++) {
                for (int p = 0; p < b.length; p++) {
                  for (int l = 0; l < instanceReplications; l++) {
                    ...
                    flowshop1.setParameter(crossoverRate[m], mutationRate[n], counter, elitism[o], generations[0], length, fs.getfileName(),
                            fs.getPiTotal(), fs.getMachineTotal(), fs.getprofit(), fs.getdi(), fs.getwi(), fs.getprocessingTime(), b[0], latch);
                    executor.execute(flowshop1);
                    counter++;
                  }
                }
