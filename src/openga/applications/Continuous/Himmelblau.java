...
public class Himmelblau {
  ...
  public void initiateVars(){
    ...
    ObjectiveFunction[0] = new ObjectiveFunctionContinuous();//the first objective, Dij*fij
...
*/
  ...
  public static void main(String[] args) {
    ...
    System.out.println("Start to calc the Himmelblau function, the following number is the number of combination");
    String implementResult = "counter\t seed\t crossoverRate\t mutationRate\t X1\t X2\t\t objValue\t time\n";
    Himmelblau1.writeFile("Continuous", implementResult);
    //to test different kinds of combinations.
    for(int i = 0 ; i < seed.length ; i ++ ){
      for(int j = 0 ; j < crossoverRate.length ; j ++ ){
        for(int k = 0 ; k < mutationRate.length ; k ++ ){
          System.out.println(counter);
          Himmelblau1.setParameter(seed[i], crossoverRate[j], mutationRate[k], counter);
          Himmelblau1.initiateVars();
          Himmelblau1.start();
          counter ++;
        }
      }
