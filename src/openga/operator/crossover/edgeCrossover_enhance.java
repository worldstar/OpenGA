 ...
 * @author nhu
 */
public class edgeCrossover_enhance implements CrossoverI{
    public edgeCrossover_enhance() {
    }
    ...
    int edge[][]=new int[500][500];  //this is map table(chromosomeLength*chromosomeLength)
    ...
    public void startCrossover(){
        /*
        for(int i = 0 ; i < popSize ; i ++ ){
            maptable(getCrossoverChromosome(i));
        }
         */
        for(int i = 0 ; i < popSize ; i ++ ){
    ...
    public int[] getCrossoverChromosome(int sourceParentID){
    ...
    private void maptable(int[] indexes){
        int pre_pos, pos, next_pos;
        ...
        for(int j = 0 ; j < this.chromosomeLength ; j ++){
            ...
            edge[newPop.getSingleChromosome(indexes[0]).genes[pos]][newPop.getSingleChromosome(indexes[0]).genes[next_pos]]++;
            edge[newPop.getSingleChromosome(indexes[0]).genes[pos]][newPop.getSingleChromosome(indexes[0]).genes[pre_pos]]++;
//                System.out.println(newPop.getSingleChromosome(indexes[0]).genes[pos] + "-->"+newPop.getSingleChromosome(indexes[0]).genes[next_pos]);
//             System.out.println(edge[newPop.getSingleChromosome(indexes[0]).genes[pos]][newPop.getSingleChromosome(indexes[0]).genes[next_pos]]);
        }
    ...
    private void copyElements(int[] indexes, int numberOfParents){
        ...
        //test data
 /*
        newPop.setGene(indexes[0], 0, 0);
        newPop.setGene(indexes[0], 1, 1);
        newPop.setGene(indexes[0], 2, 2);
        newPop.setGene(indexes[0], 3, 3);
        newPop.setGene(indexes[0], 4, 4);
        newPop.setGene(indexes[0], 5, 5);
        newPop.setGene(indexes[0], 6, 6);
        newPop.setGene(indexes[0], 7, 7);
        this.chromosomeLength =8;
        newPop.setGene(indexes[1], 0, 1);
        newPop.setGene(indexes[1], 1, 0);
        newPop.setGene(indexes[1], 2, 5);
        newPop.setGene(indexes[1], 3, 2);
        newPop.setGene(indexes[1], 4, 3);
        newPop.setGene(indexes[1], 5, 7);
        newPop.setGene(indexes[1], 6, 6);
        newPop.setGene(indexes[1], 7, 4);
  */
        
  /*      
        for(int jj = 0 ; jj < indexes.length ; jj ++){          // print all parents chromsome
   ...
   */     
        ...
        for(int i = 0 ; i < indexes.length; i ++){                    //create mapping table
            for(int j = 0 ; j < this.chromosomeLength ; j ++){
                ...
                edge[newPop.getSingleChromosome(indexes[i]).genes[pos]][newPop.getSingleChromosome(indexes[i]).genes[next_pos]]++;
                edge[newPop.getSingleChromosome(indexes[i]).genes[pos]][newPop.getSingleChromosome(indexes[i]).genes[pre_pos]]++;
            }
        }
  ...
  /*      
        System.out.println("********************************************************");     //print edge table
  ...
  */      
  /*      
        System.out.println("-----------------------------------------------------------");    //print sum table
        for(int jj = 0 ; jj < this.chromosomeLength ; jj ++){
  ...
  */      
        // System.exit(0);
 /*
        for (int ff = 0; ff< this.chromosomeLength-1 ;ff++){            //sort sum table decrease
 ...
 */
        //       System.exit(0);
// create link from first element of parents, select element from edge table with maximun value
//if second element is selected from the smallest set of link first element and mark it by -1
//        newPop.setGene(indexes[0], 0, s_i);
//if can not find the element(all is -1), then select the smallest unmark element
        
        s_i = (int)(Math.random() * this.chromosomeLength);
        //s_i =newPop.getSingleChromosome(indexes[0]).genes[0] ;
        sum[s_i][0] = -1;
        counter = 0;
...
//            System.out.print(s_i+"----->");
            for(int j = 0 ; j < this.chromosomeLength ; j++){  //search link
                if(edge[s_i][j]!=0){
//                    System.out.print( j+"("+edge[s_i][j] +")(" + sum[j][0]+")(" + sum[j][1]+"),");
                    if (sum[j][0] != -1){
                        if  (edge[s_i][j]>counter){
                            counter = edge[s_i][j];
                            tmp_edge = j;
                        }else if (edge[s_i][j]==counter){
                            if (sum[j][1]<sum[tmp_edge][1]){
                                tmp_edge = j;
                            }else if (sum[j][1]==sum[tmp_edge][1]){
            ...
            //System.exit(0);
            if(tmp_edge==this.chromosomeLength){
                for(int n_i = 0; n_i<this.chromosomeLength; n_i++){
                    if(sum[n_i][0]!=-1){
                        tmp_edge= sum[n_i][0];
                        break;
                    }
...
//            System.out.println("==>"+ tmp_edge);
            s_i = tmp_edge;
            newPop.setGene(indexes[0], i, s_i);
            sum[tmp_edge][0] = -1;
            tmp_edge = this.chromosomeLength;
            counter = 0;
        }
/*
        System.out.println();
