...
public class parallelMachineSetupGA2 extends parallelMachineSetupGA {
  public parallelMachineSetupGA2() {
  }
  double GADPCPUTIME = 0, DPCPUTIME = 0;

  public void initiateVars(){
    GaMain     = new singleThreadGAwithInitialPop2();//singleThreadGAwithSecondFront singleThreadGAwithInitialPop
  ...
  public void startMain(){
    ...
    writeFile("ParallelMachineSetupMDP_20070530", implementationResult);
  ...
  public void constructInitialSolutions2(populationI _Population){
    for(int i = 0 ; i < DEFAULT_PopSize ; i ++ ){
      parallelMachineSetup parallelMachineSetup1 = new parallelMachineSetup();
      parallelMachineSetup1.setData(numberOfJob, numberOfMachines, processingSetupTime, _Population.getSingleChromosome(i).getSolution());
      //parallelMachineSetup1.generateInitialSolution(1);
      parallelMachineSetup1.setIterations(2);
      parallelMachineSetup1.startAlgorithm();
      _Population.getSingleChromosome(i).setSolution(parallelMachineSetup1.getSolution());
      ...
      for(int n = 0 ; n < numberOfMachines.length ; n ++ ){
        for(int p = 0 ; p < type.length ; p ++ ){
          for(int k = startInstanceID ; k <= endStartInstanceID ; k ++ ){//instance replications
            ...
            String fileName = "instances\\ParallelMachineSetup\\"+type[p]+"\\"+numberOfMachines[n]
                +"machines\\"+numberOfJobs[m]+"on"+numberOfMachines[n]+"Rp50Rs50_"+k+".dat";
            ...
            for(int replications = 0 ; replications < totoalReplications ; replications ++ ){
              parallelMachineSetupGA2 parallelMachineSetupDP1 = new parallelMachineSetupGA2();
