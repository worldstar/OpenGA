...
public class NEH_Method{
    public int[] startNEH(int[][] processingTime, int numberOfJob, int numberOfMachines){
        ...
        */
        ...
        for(int i = 0 ; i < size ; i ++){
            sequence[i] = list.get(size-i-1);
        }
        
        int makespan_final = calcObjectives(sequence, processingTime, numberOfJob, numberOfMachines);
        System.out.print(makespan_final+"\n");
        System.exit(0);
        writeFile("NEH_all" , makespan_final + "\n");   
    ...
    public int calcObjectives(int[] partc,int[][] processingTime, int numberOfJob, int numberOfMachines) {
        int machineTime[] = new int[numberOfMachines];
        int objVal = 0;
