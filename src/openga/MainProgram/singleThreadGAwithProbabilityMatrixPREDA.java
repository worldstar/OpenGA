 ...
 */
public class singleThreadGAwithProbabilityMatrixPREDA extends singleThreadGA implements probabilityMatrixI {

    public singleThreadGAwithProbabilityMatrixPREDA() {
    }
    //PBILInteractive PBIL1;   //PBIL
    PBILInteractivejEDA PBIL2;  //jEDA
    double container[][];
    double inter[][];
    openga.operator.miningGene.probabilityMatrixInteractiveLPREDA probMatrix1 = new openga.operator.miningGene.probabilityMatrixInteractiveLPREDA();
...
//        this.processingTime = processingTime;
          System.out.println("This method setFlowShopData() is not implemented in singleThreadGAwithProbabilityMatrixPREDA.");
          System.exit(0);
    }
...
//        System.exit(0);
        ...
        PBIL2 = new PBILInteractivejEDA(Population, lamda, beta, w1, w2);   // jEDAL
...
//timeClock2.end();
            ...
            counter++;
            if (counter >= 100) {
                diversity = PBIL2.calculate_diversity();
            }
            ...
            if (diversity < 0.3) {
                ...
                //System.out.print(i+"\t"+getBest()+"\n");
                Population = initialStage();   //with initial solution must be cancel
                artificialPopulation = intialArttificalPopulation();
                evalulatePop(Population);
                //Population = ProcessObjectiveAndFitness();
                Population = replacementStage(Population, archieve);
                //System.out.print(i+"\t"+getBest()+"\n");
                diversity = 1;
                counter = 0;
                //System.exit(0);
            } else {
                artificialPopulation = selectionStage(Population);
                PBIL2.setData(artificialPopulation);
                PBIL2.startStatistics();     //notice best number
                container = PBIL2.getContainer();
                inter = PBIL2.getInter();
                probMatrix1.setData(Population, artificialPopulation, container, inter, w1, w2);//archieve, Population, artificialPopulation
                probMatrix1.setDuplicateRate(0.05);  //
            ...
            if (i == 500) {
                ...
                generationResults = i + "\t" +  getBest() + "\n";
                //System.out.print(generationResults);
                writeFile("PREDA1002_" + i, generationResults);
            }
            if (i == 750) {
                String generationResults = "";
                generationResults = i + "\t" + getBest() + "\n";
...
//           System.exit(0);
    ...
    public void popcopy(populationI Population1, populationI Population2) {

        for (int i = 0; i < Population1.getPopulationSize(); i++) {

            Population2.setSingleChromosome(i, Population1.getSingleChromosome(i));

        }
        //System.out.println( numberOfOverlapped);
    }
...
//
    ...
    public populationI intialArttificalPopulation() {
        artificialPopulation.setGenotypeSizeAndLength(encodeType, fixPopSize, length, numberOfObjs);
        artificialPopulation.createNewPop();
        return artificialPopulation;
    }
...
//        localSearch1.startLocalSearch();
//        currentUsedSolution = localSearch1.getCurrentUsedSolution();
          System.out.println("This method localSearchStageDC() is not implemented in singleThreadGAwithProbabilityMatrixPREDA.");
          System.exit(0);
    }
