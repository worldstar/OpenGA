 ...
 * @author nhu
 */
public class edgeCrossover implements CrossoverI{
...
*/       
        ...
        for(int fir = 0; fir<this.chromosomeLength; fir++){
            if (sum[fir][0]==s_i){
                sum[fir][0] = -1;
            }
        }
...
// create link from first element of parents
//second element is selected from the smallest set of link first element and mark it by -1
//        newPop.setGene(indexes[0], 0, s_i);        //if can not find the element(all is -1), then select the smallest unmark element
        
        for(int i =1; i< this.chromosomeLength; i++){
            sum_pos = this.chromosomeLength;
//            System.out.print(s_i+"----->");
            for(int j = 0 ; j < this.chromosomeLength ; j++){  //search link
                if (edge[s_i][j]==1){
//                    System.out.print( j +",");                  
                    for (int k =0; k<this.chromosomeLength ; k++){  //search min_element from random table
                        if ((sum[k][0]==j)&&(sum[k][0]!=-1)){
                            if (k<sum_pos){
                                sum_pos = k;
                            }
                            break;
            ...
            //System.exit(0);
            if(sum_pos==this.chromosomeLength){
...
//            System.out.println("result: "+sum[sum_pos][0] + ",");
            s_i = sum[sum_pos][0];
